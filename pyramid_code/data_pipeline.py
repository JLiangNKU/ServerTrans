...
class ImageFilesDataPipeline_unpair(DataPipeline):
  """Pipeline to process pairs of images from a list of image files.
  ...
  def _produce_one_sample(self):
    ...
    input_files = [os.path.join(dirname, f) for f in flist]

    self.nsamples = len(input_files)

    input_queue, _ = tf.train.slice_input_producer(
        [input_files, input_files], shuffle=self.shuffle,
        seed=123, num_epochs=self.num_epochs)
    ...
    inout = im_input
    fullres, inout = self._augment_data(inout, 3)

    sample['lowres'] = inout
    sample['fullres'] = fullres
