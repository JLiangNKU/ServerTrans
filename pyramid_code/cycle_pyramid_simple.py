...
from model_pyramid_norm import Generator, Discriminator, cycle_consistency_loss, generator_loss, discriminator_loss
...
req_grp.add_argument('--model_save_dir', default='../cycle_simple_check', type=str, help='directory to save checkpoints to.')
req_grp.add_argument('--task', default='sum2win', type=str, help='task to do')
req_grp.add_argument('--save_dir_train', default='../cycle_simple_train', type=str, help=' ')
req_grp.add_argument('--save_dir_test', default='../cycle_simple_test', type=str, help=' ')
...
def train(train_datasetA, train_datasetB, args):
    ...
    for iteration in range(args.start_iter, args.max_iterations):
        ...
        with tf.GradientTape() as genA2B_tape, tf.GradientTape() as genB2A_tape, \
            ...
            genA2B_output = genA2B(trainA, trainB, training=True)
            genB2A_output = genB2A(trainB, trainA, training=True)

            genA2B_output_full = merge_image(genA2B_output, split_list_A)
            genB2A_output_full = merge_image(genB2A_output, split_list_B)

            reconstructed_A_low = genB2A(genA2B_output, genB2A_output, training=True)
            reconstructed_B_low = genA2B(genB2A_output, genA2B_output, training=True)
            ...
            generatorA2B_loss = generator_loss(discB_fake_output)
            ...
            generatorB2A_loss = generator_loss(discA_fake_output)
        ...
        if iteration % args.log_interval == 0:
            ...
            print('Training ' + procname + ', Iteration: {}th, time: {:.4f}, LOSSES: cycle: {:.4f}'.format(iteration, time.time() - start, cycleA2B_loss))

            start = time.time()

        if iteration % args.save_interval == 0:
            generate_images(trainA_full, trainB_full, genA2B_output_full, genB2A_output_full, args.save_dir_train, iteration)
        ...
        if iteration % args.test_interval == 0:
            ...
            for _ in range(args.test_size):
                ...
                generated_low = genA2B(test_low, trainB, training=False)
