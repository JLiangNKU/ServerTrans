...
import datetime
from model_pyramid_unpair import *
    ...
    for iteration in range(args.start_iter, args.max_iterations):
        ...
        if iteration % args.test_interval == 0:
            ...
            for i in range(args.test_size):
                ...
                test_input = next(test_datasetB)

                multi_test_save_single = np.vstack((multi_test_save_single, tf.reshape(tf.clip_by_value((test_input+1)/2, 0, 1), [args.load_img_size, args.load_img_size, 3]).numpy()))
                ...
                for i__ in range(1, args.level+1):
