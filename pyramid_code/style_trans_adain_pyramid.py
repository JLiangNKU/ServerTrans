...
req_grp.add_argument('--prefix', default='../styletrans_mysum2win', type=str, help='prefix of this training')
...
def train(train_datasetA, train_datasetB, args):
    ...
    for iteration in range(args.start_iter, args.max_iterations):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape, tf.GradientTape() as trans_tape:
            ...
            _, lows_B = pyramid.split(trainB_full, args.level)
            
            genA2B_output = gen(lows_A[-1], lows_B[-1])
        ...
        if iteration % args.test_interval == 0:
            ...
            for i in range(args.test_size):
                ...
                test_B_unpair = next(train_datasetB)
                ...
                _, lows_B = pyramid.split(test_B_unpair, args.level)
                generated_low = gen(lows_test[-1], lows_B[-1])
                ...
                multi_test_save_single = np.vstack((multi_test_save_single, tf.reshape(tf.clip_by_value((test_B_unpair+1)/2, 0, 1), [args.load_img_size, args.load_img_size, 3]).numpy()))
        ...
        #         test_input, test_output = next(test_dataset)
        #         test_B_unpair = next(test_datasetA)
