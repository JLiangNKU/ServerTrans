...
req_grp.add_argument('--prefix', default='../day2night_dischigh', type=str, help='prefix of this training')
...
train_grp.add_argument('--recons_lambda', default=1, type=float, help='weight of the cycle loss.')
...
train_grp.add_argument('--save_interval', type=int, default=100, help='interval between saving model checkpoints')
...
disc_high_optimizer = keras.optimizers.Adam(args.learning_rate, beta_1=0.5)
...
def train(train_datasetA, train_datasetB, args):
    ...
    for iteration in range(args.start_iter, args.max_iterations):
        ...
        with tf.GradientTape() as gen_tape, tf.GradientTape() as disc_tape, tf.GradientTape() as disc_high_tape, tf.GradientTape() as trans_tape:
            try:
            ...
            highs_B, _ = pyramid.split(trainB_full, args.level)
            ...
            # disc_real_high = disc_high([highs_B[0], highs_B[1], highs_B[2]])
            # disc_fake_high = disc_high([split_list_A[0], split_list_A[1], split_list_A[2]])

            # disc_loss_high = discriminator_losssss('lsgan', disc_real_high, disc_fake_high)
            # generator_loss_high = generator_losssss('lsgan', disc_fake_high)
            ...
            trans_loss = args.recons_lambda * reconstruction_loss # + args.dis_lambda * generator_loss_high

        gen_gradients = gen_tape.gradient(gen_loss, gen.trainable_variables)
        trans_gradients = trans_tape.gradient(trans_loss, trans.trainable_variables)
        disc_gradients = disc_tape.gradient(disc_loss, disc.trainable_variables)
        # disc_high_gradients = disc_high_tape.gradient(disc_loss_high, disc_high.trainable_variables)
        ...
        # disc_high_optimizer.apply_gradients(zip(disc_high_gradients, disc_high.trainable_variables))

        for var in disc.trainable_variables:
            # print(var)
