...
class Transform_mask_block(keras.Model):

    def __init__(self):
        super(Transform_mask_block, self).__init__()

        self.conv1 = keras.layers.Conv2D(16, kernel_size=3, strides=1,
                                            kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        self.conv2 = keras.layers.Conv2D(1, kernel_size=3, strides=1,
                                            kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        # self.conv3 = keras.layers.Conv2D(16, kernel_size=3, strides=1,
        #                                     kernel_initializer=tf.random_normal_initializer(stddev=0.02))
        # self.conv4 = keras.layers.Conv2D(1, kernel_size=3, strides=1,
        #                                     kernel_initializer=tf.random_normal_initializer(stddev=0.02))
...
class Transform_high(keras.Model):
    ...
    def __init__(self):
        ...
        self.trans_mask_block_1 = Transform_mask_block()
        self.trans_mask_block_2 = Transform_mask_block()
        self.trans_mask_block_3 = Transform_mask_block()
    ...
    def call(self, inputs):
        ...
        x = tf.pad(x, [[0, 0], [1, 1], [1, 1], [0, 0]], "REFLECT")
        mask_0 = self.conv_last(x)
        # mask_0 = tf.nn.tanh(x)

        mask_1 = up_sample(mask_0)
        mask_1 = self.trans_mask_block_1(mask_1)

        mask_2 = up_sample(mask_1)
        mask_2 = self.trans_mask_block_2(mask_2)

        mask_3 = up_sample(mask_2)
...
class Discriminator_high(keras.Model):
