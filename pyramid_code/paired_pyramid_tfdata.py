...
def train(train_dataset, args, lsgan=True):
    ...
    for iteration in range(args.start_iter, args.max_iterations):
        ...
        with tf.GradientTape() as genA2B_tape, tf.GradientTape() as transA2B_tape:
            ...
            genA2B_loss = args.mse_lambda_low * mse_loss_low + args.mse_lambda_full * mse_loss_full + args.mse_lambda_high * mse_loss_high# + color_loss# + args.tv_lambda * tv_loss
        ...
        if iteration % args.log_interval == 0:
            ...
            print('Training ' + procname + ', Iteration: {}th, time: {:.4f}, LOSSES: mse_low: {:.4f}, mse_high: {:.4f}, mse_full: {:.4f}, psnr: {:.4f}'.format(
        ...
        if iteration % args.test_psnr_interval == 0:
            ...
            print('test iteration: {}th, mean psnr: {:.4f}, avg inference time: {:.4f}'.format(iteration, np.mean(psnr_all), np.mean(cost_all)))
            print('****************************************************************************')
