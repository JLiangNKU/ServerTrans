...
parser.add_argument("--dataset_name", type=str, default="sum2win", help="name of the dataset")
...
parser.add_argument("--img_height", type=int, default=3840, help="size of image height")
parser.add_argument("--img_width", type=int, default=3840, help="size of image width")
...
parser.add_argument('--validate_size', default=1, type=int, help='number of images to validate.')
...
# G = GeneratorResNet_original(input_shape, opt.n_residual_blocks)
# D = MultiDiscriminator()
Trans = Trans_high()
# Trans = ResidualBlock_high_original(input_shape, 3)
...
if opt.epoch != 0:
    ...
    G.load_state_dict(torch.load("../saved_models3/%s/G_%d.pth" % (opt.dataset_name, opt.epoch - 1)))
   # D.load_state_dict(torch.load("../saved_models3/%s/D_%d.pth" % (opt.dataset_name, opt.epoch - 1)))
    Trans.load_state_dict(torch.load("../saved_models3/%s/Trans_%d.pth" % (opt.dataset_name, opt.epoch - 1)))
else:
    # Initialize weights
...
optimizer_Trans = torch.optim.Adam(Trans.parameters(), lr=opt.lr * 0.1, betas=(opt.b1, opt.b2)
)
...
def pyramid_transform(pyr_original, real_low, fake_low, trans_net, if_conv):
    ...
    mask_0, mask_1, mask_2 = trans_net(pyr_original[-2], if_conv)
...
def pyramid_transform_simplify(pyr_original, real_low, fake_low, trans_net, if_conv):
    pyr_result = pyr_original
    pyr_result[-1] = fake_low
...
def sample_images(batches_done):
    ...
    # pyr_trans = pyramid_transform_simplify(pyr, pyr[-1], fake, Trans, if_conv)
    ...
    if not os.path.exists("../images3/%s" % (opt.dataset_name)):
        os.makedirs("../images3/%s" % (opt.dataset_name))
    save_image(image_grid, "../images3/%s/%s.png" % (opt.dataset_name, batches_done), normalize=False)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # pyr_A_trans = pyramid_transform_simplify(pyr_A, real_A, fake_B, Trans, if_conv)
        fake_B_full = pyramid.pyramid_recons(pyr_A_trans)
        # fake_B_full = torch.clamp(fake_B_full, -1, 1)

        loss_GAN = D.compute_loss(fake_B_full, valid)
        loss_recons = criterion_cycle(fake_B_full, real_A_full)
        loss_G = 1 * loss_GAN + 5 * loss_recons
        ...
        loss_Trans = 1 * loss_GAN + 10 * loss_recons
        loss_Trans.backward()
        ...
        if i % 1 == 0:
            ...
            # Real loss
            loss_real = D.compute_loss(real_B_full, valid)
            # Fake loss (on batch of previously generated samples)
            fake_B_ = fake_B_buffer.push_and_pop(fake_B_full)
            loss_fake = D.compute_loss(fake_B_.detach(), fake)
            # Total loss
    ...
    if opt.checkpoint_interval != -1 and epoch % opt.checkpoint_interval == 0:
        ...
        if not os.path.exists("../saved_models3/%s" % (opt.dataset_name)):
            os.makedirs("../saved_models3/%s" % (opt.dataset_name))
        torch.save(G.state_dict(), "../saved_models3/%s/G_%d.pth" % (opt.dataset_name, epoch))
        torch.save(D.state_dict(), "../saved_models3/%s/D_%d.pth" % (opt.dataset_name, epoch))
        torch.save(Trans.state_dict(), "../saved_models3/%s/Trans_%d.pth" % (opt.dataset_name, epoch))
