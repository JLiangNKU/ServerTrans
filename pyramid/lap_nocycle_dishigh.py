...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        loss_GAN = D.compute_loss(fake_B_full, valid) + D.compute_loss(pyr_A_trans[0], valid) + D.compute_loss(pyr_A_trans[-1], valid)
        loss_recons = criterion_cycle(fake_B_full, real_A_full)
        loss_G = 10 * loss_GAN + 1 * loss_recons
        ...
        loss_Trans = 5 * loss_GAN + 1 * loss_recons
        loss_Trans.backward()
        ...
        if i % 2 == 0:
            ...
            # Real loss
            loss_real = D.compute_loss(real_B_full, valid) + D.compute_loss(pyr_B[0], valid) + D.compute_loss(pyr_B[-1], valid)
