...
def main():
    ...
    parser.add_argument('-opt', type=str, default='options/train/train_AI4K_EDVR_L.yml', help='Path to option YAML file.')
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #### calculate PSNR using cuda
            mse = mseloss(model.fake_H, model.real_H)
            psnr_sum_train += 10 * math.log10(1.0 / (mse.item() + 1e-8))

            #visuals = model.get_current_visuals()
            #rlt_img = util.tensor2img(visuals['rlt'])  # uint8
            #gt_img = util.tensor2img(visuals['GT'])  # uint8
            #psnr_sum_train += util.calculate_psnr(rlt_img, gt_img)
            ...
            if opt['datasets'].get('val', None) and current_step % opt['train']['val_freq'] == 0:
                if opt['model'] in ['sr', 'srgan'] and rank <= 0:  # image restoration validation
                    ...
                    if opt['dist']:
                        ...
                        pbar = util.ProgressBar(len(val_loader))
                        psnr_sum, psnr_avg = 0., 0.
                        ssim_sum, ssim_avg = 0., 0.
                        for val_data in val_loader:
                            model.feed_data(val_data)
                            model.test()
                            visuals = model.get_current_visuals()
                            ...
                            ssim_sum += util.calculate_ssim(rlt_img, gt_img)
                            pbar.update()
                        psnr_avg = psnr_sum / len(val_loader)
                        ssim_avg = ssim_sum / len(val_loader)

                        # print out information
                        logger.info('# Validation # PSNR: {:.2f} dB'.format(psnr_avg))
                        logger.info('# Validation # SSIM: {:.4f}'.format(ssim_avg))

                        # write to tensorboard
                        if opt['use_tb_logger'] and 'debug' not in opt['name']:
                            tb_logger.add_scalar('psnr_avg', psnr_avg, current_step)
                            tb_logger.add_scalar('ssim_avg', ssim_avg, current_step)
