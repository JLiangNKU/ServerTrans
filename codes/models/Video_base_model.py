...
class VideoBaseModel(BaseModel):
    ...
    def optimize_parameters(self, step):
        ...
        if which_model == 'EDVR':
            ...
            self.fake_H, self.denoised_L = self.netG(self.var_L)
            self.resized_real_H = nn.functional.interpolate(self.real_H, scale_factor=0.25, mode='bilinear', align_corners=False)
            l_pix = self.l_pix_w * self.cri_pix(self.fake_H, self.real_H) + self.cri_pix(self.denoised_L, self.resized_real_H)
        else:
            self.fake_H, self.fake_x2 = self.netG(self.var_L)
