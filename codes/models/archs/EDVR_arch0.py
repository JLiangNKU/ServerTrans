...
class EDVR_DN(nn.Module):

    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
                 predeblur=False, HR_in=False, w_TSA=False):
        super(EDVR_DN, self).__init__()
        self.nf = nf
        self.center = nframes // 2 if center is None else center
        self.is_predeblur = True if predeblur else False
        self.HR_in = True if HR_in else False
        ...
        ## denoise
        self.denoise_residual = arch_util.make_layer(ResidualBlock_noBN_f, front_RBs)
        self.denoise_output = nn.Conv2d(64, 3, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        denoised_fea = self.denoise_residual(aligned_fea.view(-1, self.nf, H, W))
        denoised_out = self.denoise_output(denoised_fea)
        denoised_out = denoised_out.view(B, N, -1, H, W)
        denoised_out = denoised_out[:, self.center, :, :, :].contiguous()
        denoised_fea = denoised_fea.view(B, N, -1, H, W)

        if not self.w_TSA:
            denoised_fea = denoised_fea.view(B, -1, H, W)
        fea = self.tsa_fusion(denoised_fea)
        ...
        if self.HR_in:
            ...
            base = F.interpolate(denoised_out, scale_factor=4, mode='bilinear', align_corners=False)
        out += base
        return out, denoised_out

class EDVR_pyramid(nn.Module):
    ...
    def __init__(self, nf=64, nframes=5, groups=8, front_RBs=5, back_RBs=10, center=None,
        ...
        ## x2 SR
        self.x2_residual = arch_util.make_layer(ResidualBlock_noBN_f, front_RBs)
    ...
    def forward(self, x):
        ...
        fea_x2 = self.recon_trunk1(fea)
        fea_x2 = self.lrelu(self.pixel_shuffle(self.upconv1(fea_x2)))
        fea_x2 = self.lrelu(self.HRconv1(fea_x2))
        out_x2 = self.conv_last(fea_x2)
        out_x2 += F.interpolate(x_center, scale_factor=2, mode='bilinear', align_corners=False)

        out_x4 = self.recon_trunk2(fea_x2)
        out_x4 = self.lrelu(self.pixel_shuffle(self.upconv2(out_x4)))
        out_x4 = self.lrelu(self.HRconv2(out_x4))
        out_x4 = self.conv_last(out_x4)
        out_x4 += F.interpolate(out_x2, scale_factor=2, mode='bilinear', align_corners=False)
        return out_x4, out_x2


if __name__ == '__main__':
    device = torch.device('cuda')
    model = EDVR_DN(nframes=5).to(device)
    x = torch.randn(2, 5, 3, 64, 64).to(device)
    out = model(x)
    print(out.shape)
